...
export default async function DashboardPage() {
  ...
  const [
    ...
    supabase
      .from("invoices")
      .select("id, total_amount, amount_paid, status, issue_date, due_date")
  ...
  const paidInvoices =
    invoicesResult.data?.filter(
      (inv) => Number(inv.amount_paid) >= Number(inv.total_amount),
    ).length || 0;
  const totalRevenue =
    paymentsResult.data
      ...
      <div className="w-full p-4 sm:p-6 lg:p-8">
        <div className="mb-6 px-0 sm:px-0">
          <p className="text-xs sm:text-sm text-muted-foreground">
            Showing data for Financial Year:{" "}
